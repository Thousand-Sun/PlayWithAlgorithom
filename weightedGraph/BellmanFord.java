...
public class BellmanFord<Weight extends Number & Comparable> {
    ...
    public BellmanFord(Graph graph, int s){
        ...
        for( int pass = 1 ; pass < G.V() ; pass ++ ){
            ...
            for( int i = 0 ; i < G.V() ; i ++ ){
                ...
                for( Object item : G.adjIterator(i) ){
                    Edge<Weight> e = (Edge<Weight>)item;
                    // 对于每一个边首先判断e->v()可达
                    // 之后看如果e->w()以前没有到达过， 显然我们可以更新distTo[e->w()]
                    // 或者e->w()以前虽然到达过, 但是通过这个e我们可以获得一个更短的距离, 即可以进行一次松弛操作, 我们也可以更新distTo[e->w()]
                    if( from[e.v()] != null && (from[e.w()] == null || distTo[e.v()].doubleValue() + e.getWeight().doubleValue() < distTo[e.w()].doubleValue()) ){
                        distTo[e.w()] = distTo[e.v()].doubleValue() + e.getWeight().doubleValue();
                        from[e.w()] = e;
                    }
                }
            }
    ...
    boolean detectNegativeCycle(){
        ...
        for( int i = 0 ; i < G.V() ; i ++ ){
            for( Object item : G.adjIterator(i) ){
                Edge<Weight> e = (Edge<Weight>)item;
                if( from[e.v()] != null && distTo[e.v()].doubleValue() + e.getWeight().doubleValue() < distTo[e.w()].doubleValue() )
                    return true;
            }
        }
